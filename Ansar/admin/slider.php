...
if(isset($_GET['del'])){
	$del=$_GET['del'];
	$sql="DELETE FROM `slider` WHERE `id` = $del";
	$mysqli->query($sql);
	$message->messg("Slide Deleted");
	redirect_back();
}
  ...
  $content=$_POST['content'];
	...
	$loc2="gallery/".$time.$file_name;
			...
			if(move_uploaded_file($temp,$link)){
				...
				$sql= "INSERT INTO `slider` (`id`, `title`, `content`, `photo`) VALUES (NULL, '$title', '$content', '$loc2')";
				$query=$mysqli->query($sql);
				if($mysqli->affected_rows>0){
										
										$message->messg("Slide added succesfully");}
                    else{$message->messg("Failed to add slide, Please retry");}
										redirect_back();
									}
			}
               ...
               <div class="box-header">
                ...
                <form action="" enctype="multipart/form-data" method="post">
                  <div class="box-body">
                      ...
                      <label for="title">Header Of Post</label>
                      <input type="text" name="title" class="form-control"  placeholder="Header of post" ">
                    </div>
